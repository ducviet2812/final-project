...
export class AdminService {
    ...
    async _findWithId(_id: string): Promise<AdminDocument> {
        const result = await this.adminModel.findOne({ _id });
        if (!result) throw new NotFoundException('Admin not found');
        return result;
    }
    async findWithId(_id: string): Promise<AdminDocument> {
        return await this.adminModel.findOne({ _id });
    }
    async _findByUsername(username: string): Promise<AdminDocument> {
        const result = await this.adminModel.findOne({ username });
        if (!result) throw new NotFoundException('Admin not found');
        return result;
    }
    async findWithUsername(username: string): Promise<AdminDocument> {
